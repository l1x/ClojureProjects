...
* Let's Do Better
  ...
  fluency for errors-as-return-values with a /monad/. Despite the
...
** Fluent Functional Solution With Exceptions
  ...
  can be eliminated /via/ /memoization/, /common sub-expression
  elimination/, /lambda lifting/, /the state monad/, or /parallel
  composition/, but that is for another time and place. For now,
  write the flow directly as a sequential composition of
  function calls /via/ Clojure's \verb|->|, using its \verb|let|
...
#+END_SRC
  ...
  exception. These all use the three-term /if/-branch form, which is
...
#+END_SRC
   ...
   we must learn about monads: it puts values in boxes.
...
#+END_SRC
   ...
   and produces a value-in-a-box. Here we see again the fundamental
   simplifying idea: we're always taking values and putting them in
   boxes.
   
   The final monadic helper, /combine/, illustrates this convention.
   It is a function that, given a value from the second of the two
   web-service calls, produces a
   function-from-value-to-value-in-a-box. That function will be
   composed, via /m-bind/ with the filtered value of the first of the
   two web-service calls. 

#+NAME: monadic-helpers-3
#+BEGIN_SRC clojure :tangle no
...
#+END_SRC
   Longer chains will expand to more deeply nested compositions of
   /m-bind/, but that's behind the scenes, precisely where it should
   be. Contrast this to the Java situation, where we had no tools in
   the language for mitigating intolerable syntax. Here, we define
   our arrow directly in our application code. Without going more
   deeply into the macro language, it is at least small:
#+BEGIN_SRC clojure :tangle no
...
*** Monad Particulars and Generalities
   ...
   each monad. You package your application-dependent logic in the
   functions-from-value-to-value-in-a-boxes that you compose /via/
